...
static GtkWidget *opt_kbm_opts, *opt_selkeys, *opt_eng_ch_opts, *opt_tsin_space;
...
GtkWidget *create_tsin_space_opts()
{
  ...
  gtk_box_pack_start (GTK_BOX (hbox), opt_tsin_space, FALSE, FALSE, 0);
  ...
  gtk_combo_box_set_active (GTK_COMBO_BOX (opt_tsin_space), current_idx);
...
GtkWidget *frame_tsin_sw = gtk_frame_new(s);
  ...
  gtk_widget_set_hexpand (label_gcin_capslock_lower, TRUE);
  gtk_widget_set_halign (label_gcin_capslock_lower, GTK_ALIGN_CENTER);
...
void create_kbm_window()
{
  ...
  gtk_window_set_title (GTK_WINDOW (gcin_kbm_window), _(_L("gcin 注音/詞音/拼音設定")));
...
//  gtk_container_add (GTK_CONTAINER (frame_tsin_space_opt), );
  ...
  GtkWidget *hbox_tsin_shift_punc = gtk_hbox_new(FALSE, 0);
  gtk_box_pack_start (GTK_BOX (vbox_r), hbox_tsin_shift_punc , TRUE, TRUE, 1);
  GtkWidget *label_tsin_shift_punc = gtk_label_new(_(_L("Shift 輸入標點符號")));
  gtk_widget_set_hexpand (label_tsin_shift_punc, TRUE);
  gtk_widget_set_halign (label_tsin_shift_punc, GTK_ALIGN_CENTER);
