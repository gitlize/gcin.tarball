...
gboolean inc_tsin_use_count(TSIN_HANDLE *th, void *pho, char *ch, int prlen);
...
int get_in_area_pho_tsin_str(char *out)
{
  int i;
  int outN=0;
...
gboolean save_phrase_to_db2(CHPHO *chph, int len, gboolean en_prhase);
phokey_t utf8_pho_key(char *s)
{
  phokey_t tphokeys[32];
  tphokeys[0]=0;
  utf8_pho_keys(s, tphokeys);
  return tphokeys[0];
}
...
void save_phrase(int save_frm, int len)
{
  ...
  gboolean en_phrase = TRUE;
  int i;
  for(i=save_frm;i<=save_to;i++) {
    if (!is_legal_en_char(tss.chpho[i].ch))
       en_phrase = FALSE;

    if (tss.chpho[i].pho)
      continue;
    phokey_t tk = utf8_pho_key(tss.chpho[i].ch);
    if (!tk)
      return;
    tss.chpho[i].pho = tk;
  }

  if (!save_phrase_to_db2(&tss.chpho[save_frm], len, en_phrase)) {
    bell();
  }
...
void mask_pho_ref_tone(phokey_t *pho, phokey_t *refpho, int plen, char *tone_mask)
{
  int i;
//  dbg("mask_tone\n");
  if (!tone_mask)
...
static void get_sel_phrase0(int selidx, gboolean eqlen)
{
  ...
  extract_pho(FALSE, selidx, mlen, pp);

  dbg("pp "); prphs(pp, mlen);
...
#elif 1
    if (pinyin_s)
	  mask_pho_ref_tone(stk, pp, mlen, pinyin_s);
#endif
...
void mask_pho_ref1(phokey_t *ttt, phokey_t *key) {
	char zero=0;
	mask_pho_ref(ttt, key, 1, &zero);
}

int get_best_idx_pho_idx(int i, phokey_t key, char need_mask) {
  phokey_t ttt;
  int mcount=-1;
  int mi=-1;
...
//      dbg("\n");
	...
	char skip[]="！ ： 。 7 ；";
//	dbg(" s '%s' %x\n", s, ttt);
	if (ttt==key && mcount<count && !strstr(skip, s)) {
		mcount = count;
		mi = i;
	}
    if (ttt>key)
...
int get_idx_pho_idx(int idx, char *rneed_mark, int *ri, phokey_t *rkey) {
  ...
  *rkey = key;
  if (!key)
    return -1;
  dbg("get_idx_pho_idx %x\n", key);
  char need_mask = (pin_juyin||pho_no_tone) && !(tss.chpho[idx].flag & FLAG_CHPHO_PINYIN_TONE);
  dbg("need_mask flag:%d %d\n", tss.chpho[idx].flag, need_mask);

  *rneed_mark = need_mask;
//  dbg("need_mask %d\n", need_mask);

  int i=hash_pho[key>>9];
  *ri = get_best_idx_pho_idx(i, key, need_mask);
  if (*ri < 0) {
    return -1;
  }

  return idx_pho[*ri].start;
}
...
void extract_pho(gboolean is_en, int chpho_idx, int plen, phokey_t *pho)
{
  ...
  if (is_en) {
	...
	int tlen = 0;
    for(i=0; i < plen; i++) {
      tlen+=u8cpy(asc+tlen,  tss.chpho[chpho_idx + i].ch);
    }
  }	else {
    for(i=0; i < plen; i++) {
...
void set_chpho_ch(CHPHO *pchpho, char *utf8, int len, gboolean is_pho_phrase)
{
  ...
  for(i=0; i < len; i++) {
    ...
    if (is_pho_phrase) {
      ...
      if (!tsin_pho_mode()) {
//		dbg("set en %d\n", i);
        pchpho[i].flag |= FLAG_CHPHO_EN_PHRASE;
      }
    }
...
static gboolean pre_sel_handler(KeySym xkey, int kbstate)
{
//  dbg("pre_sel_handler\n");
  ...
  char *ppp=strchr(shift_sele,xkey);
  if (!(kbstate&LockMask) && ppp) {
	int v = ppp - shift_sele;
	KeySym k = noshi_sele[v];
	if (!phkbm.phokbm[k][0].num)
      return FALSE;
  }
...
//    draw_ul(tss.c_idx, tss.c_len);
  ...
  if (len) {
    ...
    if (len == 1) {
      hide_selections_win();
      tss.ph_sta = -1;
	  dbg("tss.c_len:%d %d\n",tss.c_len, gcin_buffer_select_char_auto_right);
...
static void tsin_create_win_save_phrase(int idx0, int len)
{
  ...
  int i;
  for(i=0;i<len;i++) {
	char *ch = tss.chpho[i+idx0].ch;
	phokey_t phs[32];
	int phsN = utf8_pho_keys(ch, phs);
	phokey_t k = tss.chpho[i+idx0].pho;
	int j;
	for(j=0;j<phsN;j++)
	  if (phs[j]==k)
		break;
...
void tsin_sel_cursor_inc_dec(gboolean is_inc)
{
...
static void pho_menu_idx_right()
{
  int N = tsin_sel_max_N();
...
#if WIN32
   if (test_mode && (xkey==XK_Shift_L||xkey==XK_Shift_R)) {
	   return 1;
   }
#endif
...
//     else
     {
       if (tss.tsin_half_full && is_ascii) {
		 ...
		 if (!en_pre_select)
           return 0;
         if (!(xkey>='A' && xkey<='Z' || xkey>='a' && xkey<='z'))
           return 0;
       }
     }
   }
   ...
   switch (xkey) {
     ...
     case XK_KP_Enter:
        if (shift_m) {
          if (!tss.c_len || tss.c_len == tss.c_idx)
            return 0;
...
tab_phrase_end:
     ...
     case XK_space:
       if (!tss.c_len && !poo.ityp3_pho && typ_pho_empty3() /* && tss.tsin_half_full */ ) {
send_space:
		 if (!tss.tsin_half_full)
			 return FALSE;
         send_text(_(_L("　")));
       ...
       if (tsin_pho_mode() && tss.c_len && (tsin_space_opt & (TSIN_SPACE_OPT_INPUT|TSIN_SPACE_OPT_FLUSH_EDIT)) && typ_pho_empty3() && !poo.ityp3_pho && !tss.sel_pho) {
//         if (tss.c_len)
           flush_tsin_buffer();
...
change_char:
     ...
     case '\'':  // single quote
       if (phkbm.phokbm[xkey][0].num && !pin_juyin || !tsin_pho_mode())
         goto other_keys;
       else {
     ...
     case ':':
     case ';':
	   if (tsin_pho_mode() || ctrl_m || tss.c_idx != tss.c_len)
         goto other_keys;
       flush_tsin_buffer();
       send_ascii(xkey);
       return TRUE;
...
clear_edit_buffer:
       ...
       use_pre_sel = tss.pre_selN && !tss.sel_pho && xkey < 127 && (!phkbm.phokbm[xkey][0].num || !tsin_pho_mode());

       char *pp;
       if ((pp=strchr(wselkey, xkey_lcase)) && (tss.sel_pho || tss.ctrl_pre_sel || use_pre_sel)) {
         int c=pp-wselkey;
...
//   dbg("key_pad %d\n", key_pad);
   ...
   if (!tsin_pho_mode() || poo.typ_pho[0]!=BACK_QUOTE_NO && (shift_m || key_pad ||
       ...
       if (tsin_pho_mode() && !shift_m && strchr(hsu_punc, xkey) && !is_pho_key(xkey)) {
		 dbg("before pre_punctuation_hsu\n");
...
asc_char:
        if (shift_m && tsin_pho_mode()) {
          if (pre_sel_handler(xkey, kbstate)) {
            call_tsin_parse();
            return 1;
          }

          if (tsin_shift_punc /* && tsin_pho_mode() */ && pre_punctuation(xkey))
            return 1;
        }

		if (!key_has_pho && tsin_pho_mode() && !tss.c_len) {
			  return FALSE;
		}
...
//        dbg("en %s\n", tstr);
		...
		if (tss.c_idx == tss.c_len && (!tsin_pho_mode() || !key_has_pho) && strchr("!@#$%^&*()-_+{}\\|;:\"<>?,./[]=", xkey)) {
			flush_tsin_buffer();
			send_text(tstr);
			return 1;
		}
...
#endif
     if (!(pin_juyin||pho_no_tone) && (ttt > key || (poo.ityp3_pho && idx_pho[vv].key!=key))) {
       while (jj<4) {
         while(kk<3)
...
//       dbg("ret a\n");
   ...
   if (!tss.c_len && poo.typ_pho[0]==BACK_QUOTE_NO && poo.stop_idx - poo.start_idx == 1)
	 ...
	 gboolean has_tone = (status&PHO_STATUS_TONE)>0;
	 if (pin_juyin || pho_no_tone) {
		int pi = get_best_idx_pho_idx(vv, key, !has_tone);
		if (pi < 0)
			put_u8_char(poo.start_idx, key, has_tone);
		else
			put_u8_char(idx_pho[pi].start, key, has_tone);
     } else
	   put_u8_char(poo.start_idx, key, has_tone);
   }
   ...
   if (status & PHO_STATUS_PINYIN_LEFT) {
	 dbg("PHO_STATUS_PINYIN_LEFT tsin\n");
     poo.ityp3_pho=0;
     disp_in_area_pho_tsin();
	 if (pin_juyin)
		pin2juyin(FALSE);
...
int feedkey_pp_release(KeySym xkey, int kbstate)
{
  ...
  switch (xkey) {
     ...
     case XK_Shift_R:
		...
		gint64 d;
		d= (ct - kpt);
		dbg("test_mode && release xkey %x %lld %lld %lld\n", xkey, ct, kpt, d);
        if (
...
gboolean save_phrase_to_db2(CHPHO *chph, int len, gboolean en_phrase)
{
   phokey_t pho[MAX_PHRASE_LEN];
   char ch[MAX_PHRASE_LEN * CH_SZ * 2];
   chpho_extract_cha(chph, len, pho, ch);

   dbg("save_phrase_to_db2 en_phrase:%d %s\n", en_phrase, ch);

   if (en_phrase) {
     return save_phrase_to_db(&en_hand, ch, NULL, len, 1);
   } else {
     return save_phrase_to_db(&tsin_hand, pho, ch, len, 1);
   }
}
